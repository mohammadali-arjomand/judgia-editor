...
class Judgia {
  ...
  async checkAnswer(caseInensitive: boolean = true): Promise<boolean | null> {
    ...
    if (this.staticAnswer) {
      const answer = caseInensitive
        ? this.staticAnswer?.toLocaleLowerCase()
        : this.staticAnswer?.toString()
...
const api = {
  ...
  execute: async (
    ...
    judgia.cppFilePath = filePath
    judgia.testcase = testcase

    return await judgia.compileAndGetStdout()
  }
}
